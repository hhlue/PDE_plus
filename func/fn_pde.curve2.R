pde.curve2<- function(x,y,slice,h1,h2,no.perm){#eg. a=pde.curve2(x,y,10,3,0.5,100)
  ...
  nx=stz.x(x)  #standardize of X
    ...
    mrsir_evl=b$evl
    mrphd_evl=b1$evl
#perm_pval=mrphd.permutation.test(x,y,b1$evl,b1$evt,no.perm)
    theta.1=b$evt[,1]  #theta.2=b$evt[,2]
    theta.2=b1$evt[,1]
...
return(list(theta=theta,phi=phi,mrsir.evl=mrsir_evl,#perm.pval=perm_pval,
mrphd.evl=mrphd_evl,g=gj,ut=u_t))
}
